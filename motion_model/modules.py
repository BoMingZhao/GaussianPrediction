...
class ImplicitAdaptiveOctantNet(nn.Module):
    ...
    def forward(self, model_input, oversample=1.0):
        ...
        # Enables us to compute gradients w.r.t. coordinates
        coords = model_input['kpts'] # (k, 3) or (k, 4)
        fine_coords = model_input['gaussians_rel_coords'] # (k, N, 3) already normalized [-1, 1]
        ...
        features_in = features.squeeze().reshape(b_size, n_channels, dx, dy, dz) # (k, c, x, y, z)
        ...
        sample_coords = fine_coords.reshape(b_size, -1, 1, 1, 3)
        # features_out = torch.nn.functional.grid_sample(features_in, sample_coords,
        #                                                mode='bilinear',
        #                                                padding_mode='border',
        ...
        # permute from (Blocks, feature_grid_size[0], patch_size**2)->(Blocks, patch_size**2, feature_grid_size[0])
        # so the network maps features to function output
        features_out = features_out.permute(2, 0, 1) # (N, k, c)
        index = index[..., None].repeat(1, 1, features_out.shape[-1]) # # (N, near_num, c)
        features_out = torch.gather(features_out, dim=1, index=index) # (N, k, 2)

        # for all spatial feature vectors, extract function value
        patch_out = self.features2sample_net(features_out) # (k, N, 1)
