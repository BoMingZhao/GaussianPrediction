...
class Load_hyper_data(Dataset):
    def __init__(self, 
        ...
        if thres_time < 1.0:
            ...
            for idx, i in enumerate(self.all_img):
                ...
                self.all_time += [time]
                if len(self.val_id) == 0:
                    if idx % 4 ==0 and time < thres_time:
                        self.i_train += [idx]
                    if (idx - 2) % 4 ==0 and time >= thres_time:
                        self.i_test += [idx]
                else:
                    if i in self.val_id and time >= thres_time:
                        self.i_test.append(idx)
                    if i in self.train_id and time < thres_time:
            ...
            assert test_time.max() >= thres_time
            assert train_time.min() < thres_time
    ...
    def load_raw(self, idx):
        ...
        image_name = os.path.basename(self.all_img[idx])
 
        w = image.size[0]
        h = image.size[1]
        time = np.array([self.all_time[idx]]).astype(np.float32)
        R = camera.orientation.T
        T = - camera.position @ R
        FovY = focal2fov(camera.focal_length, self.h)
        FovX = focal2fov(camera.focal_length, self.w)
